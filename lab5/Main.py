...
def showColoredGraph(colorlist):
    for it in range(len(colorlist)):
        print("The vertex ",it," has the color ",colorlist[it])

def vC(graph):
    utils = Utils()
    colorlist=utils.colorGraph(graph)
    showColoredGraph(colorlist)
...
def graph_options(graph) :
    ...
    while True :
        ...
        print("->lab4 for LAB4 menu")
        print("->lab5 for LAB5 menu")
        option = input(">")
        if option == 'lab1' :
            Menu.print_menuLAB1()
        elif option == 'lab2' :
